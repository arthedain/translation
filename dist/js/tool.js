...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_vue2_editor__ = __webpack_require__(12);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_vue_tabs_with_active_line__ = __webpack_require__(17);
//
//
//
//
...
/* harmony default export */ __webpack_exports__["default"] = ({
	...
	components: {
		...
		Tabs: __WEBPACK_IMPORTED_MODULE_1_vue_tabs_with_active_line__["a" /* default */]
	},
	mounted: function mounted() {
		var _this = this;

		axios.get("/nova-vendor/translation/get").then(function (response) {
	...
	methods: {
		update: function update(form) {
			...
			var editorFields = form.target.querySelectorAll(".editor-field");
			...
			try {
				for (var _iterator = editorFields[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
			...
			axios.post("/nova-vendor/translation/update", formData).then(function () {
				_this2.$toasted.show(_this2.__("Переводы успешно обновлены"), {
					type: "success"
				});
			}).catch(function (error) {
				_this2.$toasted.show(_this2.__("Ошибка"), { type: "error" });
			});
		},
		handleClick: function handleClick(newTab) {
			this.currentTab = newTab;
		},
		groupBy: function groupBy(array) {
			var result = {};
	...
	computed: {
		content: function content() {
			...
			return this.data.filter(function (item) {
				return item.key.toLowerCase().includes(_this3.search.toLowerCase());
			});
		},
    ...
    [
      _c("div", { staticClass: "py-6 px-8" }, [
        _c("input", {
          ...
          staticClass: "bg-white pl-search w-1/2 form-global-search",
          attrs: {
            type: "text",
            name: "translation-search",
            placeholder: _vm.__("Press / to search")
          },
      ...
      _c(
        "card",
        {
          staticClass: "bg-0 flex flex-col items-center justify-center",
          ...
          _c(
            ...
            [
              _c(
                ...
                _vm._l(_vm.group, function(item, key, index) {
                  return _c(
                    "div",
                    {
                      key: key,
                    ...
                    [_vm._v(_vm._s(_vm.__(key.toString())))]
                  )
                }),
                0
              ),
              _vm._v(" "),
              _vm._l(_vm.group, function(singleGroup, k, index) {
                return _c(
                  "div",
                  {
                    key: k,
                    class: [_vm.currentTab == index ? "" : "tab-body"]
                  },
                  _vm._l(singleGroup, function(item, id) {
                    return _c(
                      "div",
                      {
                        key: id,
                        staticClass: "flex py-6 px-8 items-center mb-3"
                      },
                      [
                        _c(
                          "label",
                          {
                            staticClass:
                              "inline-block text-80 pt-2 leading-tight w-1/2 mb-2",
                            attrs: { for: item.id }
                          },
                          [_vm._v(_vm._s(item[_vm.key]))]
                        ),
                        _vm._v(" "),
                        item["custom_properties"]["editor"] == false &&
                        item.value.length <= 60
                          ? _c("input", {
                              staticClass:
                                "w-full form-control form-input form-input-bordered",
                              attrs: { type: "text", name: item.id },
                        ...
                        item.value.length > 60
                          ? _c("textarea", {
                              ...
                              attrs: { name: item.id, rows: "3" },
                              domProps: { value: item[_vm.value] }
                            })
                          : _vm._e(),
                        _vm._v(" "),
                        item["custom_properties"]["editor"]
                          ? _c("vue-editor", {
                              ref: "editorvue",
                              refInFor: true,
                              staticClass: "w-full py-3 h-auto editor-field",
                              attrs: { name: item.id, value: item[_vm.value] }
                            })
                          : _vm._e()
                      ],
                      1
              ...
              _c(
                "div",
                { staticClass: "flex items-end flex-row-reverse py-6 px-8" },
                [
                  _c(
                    "button",
                    {
